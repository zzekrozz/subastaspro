import { STORAGE_KEYS } from "./site-config.js";
...
export function normalizeCompletedModules(modules, validModuleIds) {
  const validIds = new Set(validModuleIds);

  return [...new Set(modules)].filter((moduleId) => validIds.has(moduleId));
}

export function getNextIncompleteModule(modules, completedModuleIds) {
  const completedIds = new Set(completedModuleIds);

  return modules.find((module) => !completedIds.has(module.id)) || null;
}
